...
test_data = pd.read_csv('data/gaia_example_data.csv')
...
pmfield_size = 5.0 
grid_size = 96 
data, x, y = demeter.mask_data(test_data, median_ra, median_dec, gnomonic_width, pmfield_size)

class TestStringMethods(unittest.TestCase):
	def test_gnominic(self):
		x, y = demeter.to_gnomonic(test_data['ra'],test_data['dec'],median_ra, median_dec)
		ra, dec = demeter.from_gnomonic(x, y, median_ra, median_dec)
	...
	def test_make_random(self):
		randx, randy, rand_pmra, rand_pmdec = demeter.make_random_image(data, x, y, gnomonic_width, pmfield_size)
	...
	def test_find_blobs(self):
		...
		blobs, significance = demeter.find_blobs(out1, threshold=20)
	...
	def test_find_clusters(self):
		...
		stars = demeter.find_clusters(data, x, y, gnomonic_width, pmfield_size, grid_size, blobs)
